...
Difference(imOutSeuil,imOutDilatation,imOut);
char* imgDifference("09Difference4.pgm");
//imOutSeuil.save(cNomImgOut);
imOut.save(imgDifference);
...
for(int x = 0; x < imageSeuil.getHeight(); ++x)
		for(int y = 0; y < imageSeuil.getWidth(); ++y)
        {
            {

                if( imageSeuil[x][y]==255 &&imageDilater[x][y]==255){
                    imOut[x][y]=255;

                }
                 if( imageSeuil[x][y]==0 &&imageDilater[x][y]==0){
                    imOut[x][y]=255;

                }
                 if( imageSeuil[x][y]==255 &&imageDilater[x][y]==0){
                    imOut[x][y]=0;
